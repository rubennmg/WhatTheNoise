 ...
 * ******************************
 * 
 * This version adds a temporary buffer queue for storing initial data before global recording starts,
 * so initial data received by the first microphone is not lost.
 * 
 * This version has only been used for testing purposes. It still has debug messages.
 * 
 * ~ Author: rubennmg
 * 
 */
...
void *recordAudio(void *arg)
{
    ...
    bufferQueueInit(&data->tempBufferQueue);

    clock_gettime(CLOCK_MONOTONIC, &start_time);
    printf("%s ready at %ld.%09ld\n", data->micName, start_time.tv_sec, start_time.tv_nsec);
    ...
    while (!(*data->stopFlag))
    {
        ...
        if (data->recording == 0 && aboveThreshold)
        {
            ...
            bufferQueuePush(&data->tempBufferQueue, buffer, hw_timestamp);

            if (mic1Ready && mic2Ready)
            {
                globalRecording = 1;

                while (data->tempBufferQueue.front != NULL)
                {
                    BufferNode *tempNode = data->tempBufferQueue.front;
                    bufferQueuePush(&data->bufferQueue, tempNode->buffer, tempNode->timestamp);
                    data->tempBufferQueue.front = tempNode->next;
                    free(tempNode);
                    printf("%s using temporary queue\n", data->micName);
                }
                data->tempBufferQueue.rear = NULL;
            }
            ...
            if (globalRecording)
            {
                ...
                clock_gettime(CLOCK_MONOTONIC, &start_time);
                printf("%s starts new recording with hardwarwe timestamp: %ld.%09ld and system timsestamp: %ld.%09ld\n", data->micName, hw_timestamp.tv_sec, hw_timestamp.tv_nsec, start_time.tv_sec, start_time.tv_nsec);
                openFilesForRecording(data);
            }
        }
        else if (globalRecording)
        {
        ...
        if (*data->stopFlag)
        {
            ...
            if (data->recording)
            {
                ...
                globalRecording = 0;
                cleanBufferQueue(&data->bufferQueue);
                cleanBufferQueue(&data->tempBufferQueue);
            }
            pthread_mutex_unlock(data->startMutex);
            break;
        }
...
void *writeAudioToFile(void *arg)
{
    ...
    while (!(*data->stopFlag))
    {
        if (bufferQueuePop(&data->bufferQueue, buffer, &timestamp) == -1)
        {
            break;
        }
        pthread_mutex_lock(data->startMutex);
 ...
 */
void initializeMicData(MicData *data, int micNumber, pthread_mutex_t *startMutex, pthread_cond_t *startCond, int *stopFlag)
{
    data->recording = 0;
    data->fileIndex = 0;
    data->silenceCounter = 0;
...
/**
 * @brief Cleans up the main and temporary buffer queues for both microphones.
 *
 * @param dataMic1 Pointer to the MicData structure for microphone 1.
 * @param dataMic2 Pointer to the MicData structure for microphone 2.
 */
void cleanUp(MicData *dataMic1, MicData *dataMic2)
{
    ...
    cleanBufferQueue(&dataMic1->tempBufferQueue);
    cleanBufferQueue(&dataMic2->tempBufferQueue);
}
...
int main()
{
    ...
    initializeMicData(&dataMic1, 1, &startMutex, &startCond, &stopFlag);
    initializeMicData(&dataMic2, 2, &startMutex, &startCond, &stopFlag);
